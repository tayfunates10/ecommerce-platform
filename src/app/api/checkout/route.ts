...
import { checkoutErrorCodes } from "@/lib/checkout/public-error";
...
function badRequest() {
  return NextResponse.json({ ok: false, code: checkoutErrorCodes.invalidRequest }, { status: 400 });
}
...
export async function POST(request: Request) {
  if (process.env.NODE_ENV === "production") {
    return NextResponse.json(
      { ok: false, code: checkoutErrorCodes.unavailable },
      { status: 503 },
    );
  }
  ...
  if (body.locale !== "tr" && body.locale !== "en" && body.locale !== "de") return badRequest();
  if (!body.email || !/^\S+@\S+\.\S+$/.test(body.email.trim())) return badRequest();
  if (!body.fullName?.trim() || !body.address?.trim() || !body.city?.trim()) return badRequest();
  if (!body.idempotencyKey || !/^[A-Za-z0-9_-]{8,128}$/.test(body.idempotencyKey)) return badRequest();
  if (!Array.isArray(body.lines) || body.lines.length === 0 || body.lines.length > 50) return badRequest();
  ...
  if (lines.length !== body.lines.length) return badRequest();
  ...
  try {
    ...
    return NextResponse.json(
      { ok: false, code: checkoutErrorCodes.failed },
      { status: 409 },
    );
  }
}
